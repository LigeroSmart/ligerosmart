...
sub Run {
    ...
    my @StatesIDs = @{ $ConfigObject->Get('Survey::TicketStateSendCondition') };
    my $FlagCheckState = 0;
    if ( scalar @StatesIDs != 0 ) {
        for my $State (@StatesIDs) {
            $FlagCheckState = 1 if $State eq $Ticket{State};
        }
        return 1 if $FlagCheckState == 0;
    }
    else {

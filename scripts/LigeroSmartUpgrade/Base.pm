...
package scripts::LigeroSmartUpgrade::Base;    ## no critic
...
scripts::LigeroSmartUpgrade::Base - Base class for migrations.
...
Refreshes the configuration to make sure that a ZZZAAuto.pm is present after the upgrade.
    ...
    $LigeroSmartUpgradeObject->RebuildConfig(
...
Clean up the cache.
    ...
    $LigeroSmartUpgradeObject->CacheCleanup();
...
Parse and execute an XML array.
    ...
    $LigeroSmartUpgradeObject->ExecuteXMLDBArray(
        XMLArray          => \@XMLArray,
...
Parse and execute an XML string.
    ...
    $LigeroSmartUpgradeObject->ExecuteXMLDBString(
        XMLString => '
...
Checks if the given table exists in the database.
    ...
    my $Result = $LigeroSmartUpgradeObject->TableExists(
        Table => 'ticket',
    );
...
Checks if the given column exists in the given table.
    ...
    my $Result = $LigeroSmartUpgradeObject->ColumnExists(
        Table  => 'ticket',
        Column =>  'id',
    );
...
Checks if the given index exists in the given table.
    ...
    my $Result = $LigeroSmartUpgradeObject->IndexExists(
        Table => 'ticket',
        Index =>  'id',
    );
...
Clean up the cache.
    ...
    $LigeroSmartUpgradeObject->GetTaskConfig( Module => "TaskModuleName");
...
sub GetTaskConfig {
    ...
    my $File = $Home . '/scripts/LigeroSmartUpgrade/TaskConfig/' . $Param{Module} . '.yml';
...
settings by default.
    ...
    $LigeroSmartUpgradeObject->SettingUpdate(

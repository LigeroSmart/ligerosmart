...
if ( exists $Opts{h} ) {
    print <<EOF;
migrate from OTRS system to LigeroSmart.
Usage:
 migrateFromOtrs.pl -b /data_backup/<TIME>/ -d /opt/otrs/
Options:
...
if ( -e $ConfigBackupGz ) {
	...
	system("mkdir otrs_config");
	print "Restore Config.pm ...\n";
	system("tar -xzf $ConfigBackupGz -C otrs_config/ Kernel/Config.pm");
	#tar xfz Config.tar.gz -C /root/ Kernel/Config.pm
}
	...
	system("mkdir otrs_config");
	print "Restore Config.pm ...\n";
	system("tar -xjf $ConfigBackupBz2 -C otrs_config/ Kernel/Config.pm");
}
...
if ( -e $ApplicationBackupGz ) {
	...
	system("tar -xzf $ApplicationBackupGz --wildcards ./Kernel/Language/*Custom.pm");
	system("tar -xzf $ApplicationBackupGz ./Custom ./var/article");
	
}
elsif ( -e $ApplicationBackupBz2 ) {
    print "Restore $ApplicationBackupBz2 ...\n";
    #system("tar -xjf $ApplicationBackupBz2");
	system("tar -xjf $ApplicationBackupBz2 --wildcards ./Kernel/Language/*Custom.pm");
	system("tar -xjf $ApplicationBackupBz2 ./Custom ./var/article");
}

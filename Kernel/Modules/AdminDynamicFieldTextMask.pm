...
package Kernel::Modules::AdminDynamicFieldTextMask;
...
sub _AddAction {
    ...
    for my $ConfigParam (
        qw(ObjectType ObjectTypeName FieldType FieldTypeName DefaultValue ValidID Rows Cols Link LinkPreview Mask removeMaskOnSubmit clearMaskOnLostFocus clearIncomplete MaxChars CaseType InputType)
        )
    {
    ...
    if ( $GetParam{FieldType} eq 'TextMask' ) {
        $FieldConfig->{Link}        = $GetParam{Link};
        $FieldConfig->{LinkPreview} = $GetParam{LinkPreview};
        $FieldConfig->{Mask} 		= $GetParam{Mask};
        $FieldConfig->{removeMaskOnSubmit} 		= $GetParam{removeMaskOnSubmit};
        $FieldConfig->{clearMaskOnLostFocus} 	= $GetParam{clearMaskOnLostFocus};
        $FieldConfig->{clearIncomplete} 		= $GetParam{clearIncomplete};
        $FieldConfig->{MaxChars} = $GetParam{MaxChars};
        $FieldConfig->{CaseType} = $GetParam{CaseType};
        $FieldConfig->{InputType} = $GetParam{InputType};
    }
...
sub _ShowScreen {
    ...
    if ( $Param{FieldType} eq 'TextMask' ) {

		my $Opts = $ConfigObject->Get('DynamicFieldTextMask::Masks');
		for (keys %{$Opts}){
			$Opts->{$_}=$_;
		}
		$Param{MaskOption} = $LayoutObject->BuildSelection(
			Data          => $Opts,
			Name          => 'Mask',
		...
		$Param{clearMaskOnLostFocusOption} = $LayoutObject->BuildSelection(
			Data          => {
				"true"	=> "Yes",
				"false"	=> "No"
			},
			Name          => 'clearMaskOnLostFocus',
			SelectedID => $Param{clearMaskOnLostFocus} || 'false',
			PossibleNone  => 0,
			Translation   => 1,

		);
		
		$Param{removeMaskOnSubmitOption} = $LayoutObject->BuildSelection(
			Data          => {
				"true"	=> "Yes",
				"false"	=> "No"
			},
			Name          => 'removeMaskOnSubmit',
			SelectedID => $Param{removeMaskOnSubmit} || 'false',
			PossibleNone  => 0,
			Translation   => 1,

		);
		$Param{clearIncompleteOption} = $LayoutObject->BuildSelection(
			Data          => {
				"true"	=> "Yes",
				"false"	=> "No"
			},
			Name          => 'clearIncomplete',
			SelectedID => "$Param{clearIncomplete}" || 'false',
			PossibleNone  => 0,
			Translation   => 1,

		);	
		$Param{CaseTypeOption} = $LayoutObject->BuildSelection(
			Data          => {
				"0"	=> "Normal",
				"1"	=> "Upper",
				"2"	=> "Lower"
			...
			Name          => 'CaseType',
			SelectedID => "$Param{CaseType}" || '0',
			PossibleNone  => 0,
			Translation   => 1,

		);	
		$Param{InputTypeOption} = $LayoutObject->BuildSelection(
			Data          => {
				"0"	=> "Text",
				"1"	=> "Area"
			},
    ...
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AdminDynamicFieldTextMask',
        Data         => {
            %Param,

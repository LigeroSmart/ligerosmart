...
package Kernel::Modules::AgentLigeroFixCompanyCIs;
...
sub Run {
  ...
  my %Ticket = $TicketObject->TicketGet(
    ...
    my @SearchKey = (
        {
            "[1]{'Version'}[1]{'CustomerID'}[%]{'Content'}" => $Ticket{CustomerID},
        }
    );
    ...
    #    Priority => 'error',
    #    Message  => " ENTROU NO CUSTOMER CIS ".scalar @$ConfigItemIDs,
    #);
  ...
  if ( $Self->{Subaction} eq 'GetModal' ) {
    ...
    my $ModuleConfig = $LigeroFixModules->{'021-CompanyCIs'};
    ...
    my $Output = $LayoutObject->Output(
			TemplateFile => 'ModalAgentLigeroFixCompanyCIs',
			Data         => {
				%Param,
			},
		);

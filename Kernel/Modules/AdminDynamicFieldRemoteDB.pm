...
sub _AddAction {
    ...
    for my $ConfigParam (
        qw(
            ...
            ShowKeyInTitle AgentLink CustomerLink DatabaseFieldTableView
    ...
    my $FieldConfig = {
        ...
        DatabaseFieldTableView => $GetParam{DatabaseFieldTableView} || $GetParam{DatabaseFieldKey},
...
sub _ShowScreen {
    ...
    $Param{DatabaseFieldTableView} = $Param{Config}->{DatabaseFieldTableView} || $Param{Config}->{DatabaseFieldKey};
...
sub _DefaultValueSearch {
    ...
    my $DatabaseFieldTableView = $Self->{ParamObject}->GetParam( Param => 'DatabaseFieldTableView' ) || $DatabaseFieldKey;
    ...
    if (
        ...
        my $SQL = 'SELECT '
            ...
            while (my @Row = $DFRemoteDBObject->FetchrowArray()) {
                ...
                for (my $coluna = 3; $coluna <= $numColunas; $coluna++) {
                    my $valor = $Row[$coluna];
                    # Faça algo com o valor coletado, por exemplo, imprimi-lo
                    push @TableValues, $valor;
                }                

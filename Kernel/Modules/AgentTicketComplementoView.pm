...
sub new {
    ...
    $Self->{CompFilters} = $ConfigObject->Get(
        "Ticket::Frontend::AgentTicketComplementoViewFilters");
    ...
    for my $Filter ( keys %{ $Self->{CompFilters} } ) {
      ...
      # This should heappen only on first time the page is accessed
            if ( !$ParamObject->GetParam( Param => 'NotFirstRun' ) ) {
                my @Values = split /[;,]/,
                    $Self->{CompFilters}->{$Filter}->{Default};
                $Self->{$Filter} = \@Values;
            }
        }
    }
...
sub Run {
    ...
    my $StoredFilters    = $Self->{JSONObject}
        ->Decode( Data => $Preferences{$StoredFiltersKey}, );
...
COLUMNNAME:
        ...
        )
    {
        ...
        my $FilterValue
            = $ParamObject->GetParam( Param => 'ColumnFilter' . $ColumnName )
            || '';
  ...
  # if filter is not present in the web request, try with the user preferences
        if ( $FilterValue eq '' ) {
            if ( $ColumnName eq 'CustomerID' ) {
                ...
                $FilterValue
                    = $StoredFilters->{ $ColumnName . 'IDs' }->[0] || '';
            }
        }
...
DYNAMICFIELD:
    for my $DynamicFieldConfig ( @{ $Self->{DynamicField} } ) {
        ...
        my $FilterValue
            = $ParamObject->GetParam( Param => 'ColumnFilterDynamicField_'
                . $DynamicFieldConfig->{Name} );

        # if no filter from web request, try from user preferences
        if ( !defined $FilterValue || $FilterValue eq '' ) {
            $FilterValue = $StoredFilters->{ 'DynamicField_'
                    . $DynamicFieldConfig->{Name} }->{Equals};
        }
        ...
        $ColumnFilter{ 'DynamicField_' . $DynamicFieldConfig->{Name} }
            = { Equals => $FilterValue, };
        $GetColumnFilter{ 'DynamicField_' . $DynamicFieldConfig->{Name} }
            = $FilterValue;
    }
...
# COMPLEMENTO Escalation Filter                                                       #
    ...
    my %EscalationFilters = (
        ...
        1 => {
            Name   => 'Escalated',
            Prio   => 1000,
            Search => { TicketEscalationTimeOlderDate => $Now, },
        },
        2 => {
        ...
        5 => {
            ...
            Prio   => 1000,
            Search => { TicketEscalationSolutionTimeOlderDate => $Now, },
        },
        6 => {
        ...
        7 => {
            ...
            Search => {
                TicketEscalationSolutionTimeNewerDate =>
                    $TimeStampTomorrowStart,
                TicketEscalationSolutionTimeOlderDate =>
                    $TimeStampTomorrowEnd,
            },
        },
        8 => {
            ...
            Search => {
                TicketEscalationSolutionTimeNewerDate =>
                    $TimeStampNextWeekStart,
                TicketEscalationSolutionTimeOlderDate =>
                    $TimeStampNextWeekEnd,
            },
        },
        ...
        9 => {
            ...
            Prio   => 1000,
            Search => { TicketEscalationResponseTimeOlderDate => $Now, },
        },
        10 => {
        ...
        13 => {
            ...
            Prio   => 1000,
            Search => { TicketEscalationUpdateTimeOlderDate => $Now, },
        },
        14 => {
    ...
    for my $Filter ( keys %{ $Self->{CompFilters} } ) {
        if ( $Self->{$Filter} ) {
            if ( $Self->{CompFilters}->{$Filter}->{Type} eq 'TicketAttribute'
                ...
                || $Self->{CompFilters}->{$Filter}->{Type} eq
                'CustomerCompany' )
            {

                $ComplementoFilter{ $Self->{CompFilters}->{$Filter}
                        ->{FilterKey} } = $Self->{$Filter};

            } elsif (
                $Self->{CompFilters}->{$Filter}->{Type} eq 'Escalation' )
            {
...
# Disable warnings for this context
                {
                    ...
                    @ComplementoFilter{
                        keys %{
                            $EscalationFilters{ $Self->{$Filter}->[0] }
                                ->{Search}
                              }
                        }
                        = values
...
# Disable warnings for this context
    ...
    my @ViewableTickets;
    @ViewableTickets
        = $TicketObject->TicketSearch( %{ $Filters{All}->{Search} },
        %ComplementoFilter, Result => 'ARRAY', );
...
# COMPLEMENTO: Let's Count How many tickets we have per Owner, State and other attributes
    for my $TicketID (@ViewableTickets) {
        my %Ticket = $TicketObject->TicketGet(
            TicketID      => $TicketID,
            DynamicFields => 1
            , # Optional, default 0. To include the dynamic field values for this ticket on the return structure.
            UserID => $Self->{UserID},
            Silent => 0
            , # Optional, default 0. To suppress the warning if the ticket does not exist.
        );
        ...
        for my $Filter ( keys %{ $Self->{CompFilters} } ) {
            {
                no warnings 'all';
                $Counters{$Filter}
                    ->{ $Ticket{ $Self->{CompFilters}->{$Filter}->{TicketKey}
                    } }++;
            }
        }
    }
...
# COMPLEMENTO: TRANSLATE VALUES AND ADD THE COUNT AT THE END. FOR EXAMPLE: Pending (5)
        ...
        for my $Value ( keys %{ $Counters{$Filter} } ) {
            if ( $Self->{CompFilters}->{$Filter}->{Type} eq 'Agent' ) {
                ...
                $Username
                    ...
                    ->Translate('Without Owner') . ' -'
                    : $UserObject->UserName( UserID => $Value, );
                $Counters{$Filter}->{$Value}
                    = $Username . " ($Counters{$Filter}->{$Value})";
            } elsif (
                $Self->{CompFilters}->{$Filter}->{Type} eq 'CustomerCompany' )
            {
                #COMPLEMENTO: GET Company Name
                my %Company = $Self->{CustomerCompanyObject}
                    ->CustomerCompanyGet( CustomerID => $Value, );
                my $CustomerCompanyName;

                # max width
                $Param{Max} = 35;
                if (   $Company{CustomerCompanyName}
                    && $Param{Max}
                    && length $Company{CustomerCompanyName} > $Param{Max} )
                {
                    $CustomerCompanyName
                        = substr( $Company{CustomerCompanyName},
                        0, $Param{Max} - 5 )
                        . '[...]';
                    ...
                    $CustomerCompanyName
                        = '- '
                        . $LayoutObject->{LanguageObject}
                        ->Translate('Without Company') . ' -';
                }

                $Counters{$Filter}->{$Value} = $CustomerCompanyName
                    . " ($Counters{$Filter}->{$Value})";
    ...
    my $LinkSort = 'NotFirstRun=1;View='
        . $LayoutObject->Ascii2Html( Text => $Self->{View} );
    my $LinkPage
        = 'NotFirstRun=1;View='
        . $LayoutObject->Ascii2Html( Text => $Self->{View} )
        . ';SortBy='
        ...
        . ';OrderBy='
        . $LayoutObject->Ascii2Html( Text => $OrderBy ) . ';';
    ...
    for my $i ( 1 .. 4 ) {
        $EscalationFilters{$i}->{Count} = $TicketObject->TicketSearch(
            ...
            %ComplementoFilter, Result => 'COUNT',
        );
    }
    ...
    for my $Filter ( keys %{ $Self->{CompFilters} } ) {
        if ( $Self->{$Filter} ) {
            ...
            for my $Value ( @{ $Self->{$Filter} } ) {
                $LinkSort
                    .= $LayoutObject->Ascii2Html( Text => ';'
                        . $Filter . "="
                        . uri_escape( Encode::encode( "utf-8", $Value ) ) );
                $LinkFilter
    ...
    if ( $Self->{Subaction} ne 'AJAXFilterUpdate' ) {
        $Output = $LayoutObject->Header( Refresh => $Refresh, );
        $Output .= $LayoutObject->NavigationBar();
    }
...
sub _MaskQueueView {
    ...
    for my $Filter ( sort keys %{ $Self->{CompFilters} } ) {
        ...
        my $label = $Self->{CompFilters}->{$Filter}->{Label}
            || $Self->{CompFilters}->{$Filter}->{TicketKey};
...
\$('#$Filter').multiselect({
    ...
    $jsMultiple
    applyText    :'"
    . $LayoutObject->{LanguageObject}->Translate($applyText) . "',
    uncheckAllText  :'"
    . $LayoutObject->{LanguageObject}->Translate('Clear') . "',
    uncheckAll: function(){
        \$('#filters').submit();
    },
    }).multiselectfilter(
    {
        label: '"
    . $LayoutObject->{LanguageObject}->Translate('Filter') . ":',
        placeholder: '',
    });
//]]></script>
";

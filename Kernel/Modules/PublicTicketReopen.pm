...
sub Run {
    ...
    if ( !$Self->{TicketID} || !$Self->{TicketKey} ) {
        ...
        $Output .= $LayoutObject->CustomerWarning(
            ...
            Comment => Translatable('Need TicketID and TicketKey!')
        );
    ...
    # check if the ticket key is valid
    if ($Self->{TicketKey} ne $Ticket{'DynamicField_TicketKey'}) {
        my $Output = $LayoutObject->CustomerHeader( Title => 'Error' );
        $Output .= $LayoutObject->CustomerWarning(
            Message => Translatable('No permission.'),
            Comment => Translatable('Invalid Ticket Key!')
        );
        $Output .= $LayoutObject->CustomerFooter();
        return $Output;
    }

    # check if the ticket state will show the custom message
    my $StateCustomMessage = grep( $_ eq $Ticket{State},@{$Config->{TicketReopenCustomMessageTicketStates}} );
    if ($StateCustomMessage && !$Self->{ReopenSuccess}) {

        my $Output = $LayoutObject->CustomerHeader( Value => $Ticket{TicketNumber} );
        $Output   .= "<div style='" . $Config->{TicketReopenCustomMessageCSS} . "'>";
        $Output   .= $Config->{TicketReopenCustomMessage};
        $Output   .= "</div>";
        $Output   .= $LayoutObject->CustomerFooter();
        return $Output;
    }

    # check if the ticket state is valid
    #return $LayoutObject->CustomerHeader( Title => 'Error' ) . Dumper($Ticket{State});
    my $StateIsAllowed = grep( $_ eq $Ticket{State},@{$Config->{StateAllowedToReopen}} );
    if (!$StateIsAllowed && !$Self->{ReopenSuccess}) {
        my $Output = $LayoutObject->CustomerHeader( Title => 'Error' );
        $Output .= $LayoutObject->CustomerWarning(
            Message => Translatable('No permission.'),
    ...
    if ( $Self->{Subaction} eq 'Store' ) {
        ...
        # get customer data
        my %Customer = $CustomerUserObject->CustomerUserDataGet(User => $Ticket{'CustomerUserID'});
        my $From;

        if($Customer{UserEmail} ne ''){
        ...
        # remove non delivered surveys if set in sysconfig
        if ($Config->{RemoveNonDeliveredSurveysAfterReopen}) {
            $DBObject->Do(
                SQL  => "DELETE FROM survey_request WHERE ticket_id = ? AND send_time IS NULL",
                Bind => [ \$Self->{TicketID} ]
            );
        }
        ...
        return $LayoutObject->Redirect(
            OP => "Action=PublicTicketReopen;TicketID=$Self->{TicketID};TicketKey=$Self->{TicketKey};Reopen=Success",
        );
    }
...
sub _Mask {
    ...
    $Param{TicketKey}     = $Self->{TicketKey};
    $Param{ReopenSuccess} = $Self->{ReopenSuccess};
    $Param{Config}        = $Config;

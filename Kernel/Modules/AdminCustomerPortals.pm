...
package Kernel::Modules::AdminCustomerPortals;
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'Change' ) {
        ...
        my %Data = $CustomerPortalObject->CustomerPortalGet( ID => $ID );
        if ( !%Data ) {
        ...
        $Output .= $LayoutObject->Output(
            TemplateFile => 'AdminCustomerPortals',
            Data         => \%Param,
        );
    ...
    # ------------------------------------------------------------ #
        ...
        my %Data = $CustomerPortalObject->CustomerPortalGet( ID => $GetParam{ID} );
        if ( !%Data ) {
            return $LayoutObject->ErrorScreen(
                Message => Translatable('Need Type!'),
            );
        ...
        my $NameExists = $CustomerPortalObject->NameExistsCheck(
            Name => $GetParam{Name},
            ID   => $GetParam{ID}
        );
        ...
        if ( !%Errors ) {
            ...
            # update type
            my $Update = $CustomerPortalObject->CustomerPortalUpdate(
                %GetParam,
                UserID => $Self->{UserID}
            );
    ...
    # ------------------------------------------------------------ #
        ...
        # check if a type exists with this name
        my $NameExists = $CustomerPortalObject->NameExistsCheck( Name => $GetParam{Name} );
        if ($NameExists) {
            $Errors{NameExists}    = 1;
            $Errors{'NameInvalid'} = 'ServerError';
        }
        ...
        if ( !%Errors ) {
            ...
            # add type
            my $NewType = $CustomerPortalObject->CustomerPortalAdd(
                %GetParam,
                UserID => $Self->{UserID}
            );
            if ($NewType) {
...
sub _Overview {
    ...
    my $CustomerPortalObject = $Kernel::OM->Get('Kernel::System::CustomerPortal');
    my %List       = $CustomerPortalObject->CustomerPortalList( Valid => 0 );
    ...
    if (%List) {
        ...
        for my $CustomerPortalID ( sort { $List{$a} cmp $List{$b} } keys %List ) {

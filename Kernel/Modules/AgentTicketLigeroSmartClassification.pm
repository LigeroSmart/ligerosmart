...
package Kernel::Modules::AgentTicketLigeroSmartClassification;
...
sub Run {
	...
	if ($Self->{Subaction} ne 'AJAXUpdate' && $Self->{Subaction} ne 'Store'){
		if (!$Param{Show}){
		    $Param{Title} = "Smart Classification";
		    if ($Config->{WhenClassified} eq "Hide"){
		        return;
		    } else {
		        $Expand = $Config->{WhenClassified};
		    }

		} else {
		    $CssExtraClass = 'Alert';
		    $Param{Title}="This ticket is not classified yet. Check Ligero's smart suggestions and solve it now!";
		    $Expand = $Config->{WhenNotClassified};
		    $Param{NotClassified}=1;
		}
	}
# EO COMPLEMENTO
...
# EO COMPLEMENTO
	...
	############## LIGERO SMART CLASSIFICATION ####################
	# COMPLEMENTO
	# Check if we should sugest this ticket classification based on queue history
	if ($Config->{SmartClassification} && $Param{NotClassified} && $Self->{Subaction} ne 'Store' ){
        ...
        my %Search;
        my %SearchConfig = %{$Kernel::OM->Get('Kernel::Config')->Get("LigeroSmartClassification::SearchOptions")};
...
#            $Query = encode("utf-8", $Query);
        ...
        $QueryFuzzy = join(" ",@terms_new);

		# Cria o parametro de Pesquisa do ES
		my %TemplateSearchParams = (
			%Article
		);
		
        $TemplateSearchParams{"Query"} = $Query;
        $TemplateSearchParams{"QueryFuzzy"} = $QueryFuzzy;
        $TemplateSearchParams{"UnclassifiedType"} = $SearchConfig{UnclassifiedType}||'default';
        
#        $Kernel::OM->Get('Kernel::System::Log')->Log(
#            Priority => 'error',
#            Message  => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa". Dumper (\%TemplateSearchParams)
#        );
        
        $Search{params} = \%TemplateSearchParams; 
        
        $Search{inline}->{'_source'} = ["Ticket.Type","Ticket.TypeID","Ticket.QueueID","Ticket.Queue","Ticket.ServiceID","Ticket.Service","Ticket.SLA","Ticket.SLAID"];
#        Ticket.Type Ticket.TypeID Ticket.QueueID Ticket.Queue Ticket.ServiceID Ticket.Service Ticket.SLA Ticket.SLAID);
        
        # MaxResults
        $Search{inline}->{size} = 1;

        # MinScore
        #if($SearchConfig{MinScore} ne "0"){
            #$Search{min_score} = $SearchConfig{MinScore};
        #}
...
#        	$Kernel::OM->Get('Kernel::System::Log')->Log(
#        	    Priority => 'error',
#        	    Message  => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa ".$SearchResultsHash{hits}->{hits}->[0]->{_source}->{Ticket}->{SLA}
#        	);
			for my $Key( qw(TypeID ServiceID SLAID) ){
				$GetParam{"Auto$Key"}=$SearchResultsHash{hits}->{hits}->[0]->{_source}->{Ticket}->{$Key};
			}

        }
	
		# Se encontrar um resultado, preenche o GetParam com dados que foram retornados do ES
...
#        );
        $Output .= $Self->_Mask(
        #COMPLEMENTO
        	NotClassified => $Param{NotClassified}||0,
        # EO COMPLEMENTO
            TimeUnitsRequired => (
                $ConfigObject->Get('Ticket::Frontend::NeedAccountedTime')
                ? 'Validate_Required'
...
#$Kernel::OM->Get('Kernel::System::Log')->Log(
#    Priority => 'error',
#    Message  => "RTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT $Param{NotClassified}"
#);
    # get list type
    my $TreeView = 0;
	...
	# MAKE LIGERO SMART SUGESTIONS AS DEFAULT VALUES
	if($Param{NotClassified} && $Config->{SmartClassification} ){
			for my $Key( qw(TypeID ServiceID SLAID) ){
				$Param{$Key}=$Param{"Auto$Key"};
			}
    ...
    if ( $ConfigObject->Get('Ticket::Service') && $Config->{Service} ) {
        ...
        my %SLA = $TicketObject->TicketSLAList(
            %Param,
            # COMPLEMENTO
            ServiceID =>$Param{AutoServiceID} || $Param{ServiceID},
            # EO COMPLEMENTO
            Action => $Self->{Action},
            UserID => $Self->{UserID},
        );

...
package Kernel::Modules::AdminGenericInterfaceInvokerLigero;
...
$Kernel::OM->Get('Kernel::System::Log')->Log(
    ...
    my $InvokerConfig = {
        ...
        Type        => $GetParam->{InvokerType},
        PreInvoker  => $GetParam->{PreInvoker},
        PostInvoker => $GetParam->{PostInvoker},
    };
...
sub _Change {
    ...
    return $Self->_ShowScreen(
        ...
        PreInvoker      => $InvokerConfig->{PreInvoker},
        PostInvoker     => $InvokerConfig->{PostInvoker},
    );
}
...
sub _ChangeAction {
    ...
    $InvokerConfig->{PreInvoker}  = $GetParam->{PreInvoker};
    $InvokerConfig->{PostInvoker} = $GetParam->{PostInvoker};
...
sub _ShowScreen {
    ...
    # Pre and Post Web Service Invoker Call
    my @InvokerList;
    my $WSList = $Kernel::OM->Get('Kernel::System::GenericInterface::Webservice')->WebserviceList();
    if(IsHashRefWithData($WSList)){
        for my $WSID (keys %{$WSList}){
            my $WebserviceData = $Kernel::OM->Get('Kernel::System::GenericInterface::Webservice')->WebserviceGet(
                ID => $WSID,
            );
            if ( IsHashRefWithData($WebserviceData) 
                ...
                for my $Invoker (keys %{$WebserviceData->{Config}->{Requester}->{Invoker}}){
                    # Do not put the same invoker in the list
                    if($WebserviceData->{Name} ne $Param{Name} && $Invoker ne $Param{Invoker}){
                        push @InvokerList, "$WebserviceData->{Name}::$Invoker";
                    }
                }
            }
        }
    ...
    PREPOSTINVOKER:
    for my $InvokerType (qw(PreInvoker PostInvoker)) {
        my $OldInvoker = $Param{InvokerConfig}->{$InvokerType};
        my $Invoker    = $Param{$InvokerType};

        $TemplateData{ $InvokerType . 'Strg' } = $LayoutObject->BuildSelection(
            Data          => \@InvokerList,
            Name          => $InvokerType,
            SelectedValue => $Invoker,

...
sub Data {
    ...
    #
    $Self->{Translation}->{'Agent groups which can access this category.'} = 'Agenten Gruppe welche auf diese Kategorie Zugriff hat.';
    $Self->{Translation}->{'Articles'} = 'Artikel';
    $Self->{Translation}->{'Categories'} = 'Kategorien';
    $Self->{Translation}->{'Defines where the \'Insert FAQ\' link will be displayed.'} = 'Definiert wo der Link \'FAQ einfügen\' angezeigt werden soll.';
    ...
    $Self->{Translation}->{'No rate selected!'} = 'Keine Bewertung auswählt!';
    $Self->{Translation}->{'Print this FAQ'} = 'FAQ drucken';
    $Self->{Translation}->{'QuickSearch'} = 'Schnellsuche';
    $Self->{Translation}->{'SubCategories'} = 'Subkategorien';
    $Self->{Translation}->{'Thanks for your vote!'} = 'Danke für Ihre Bewertung!';
    $Self->{Translation}->{'The title is required.'} = 'Ein Titel ist erforderlich.';
    $Self->{Translation}->{'Updated'} = 'Aktualisiert';
    $Self->{Translation}->{'You have already voted!'} = 'Sie haben bereits bewertet!';

...
sub Data {
    ...
    $Self->{Translation}->{'Change Free Text of %s%s'} = '修改工单%s%s的自由字段';
    $Self->{Translation}->{'Change Owner of %s%s'} = '变更工单%s%s的所有者';
    $Self->{Translation}->{'Close %s%s'} = '关闭%s%s';
    $Self->{Translation}->{'Add Note to %s%s'} = '添加备注到工单%s%s';
    $Self->{Translation}->{'Set Pending Time for %s%s'} = '设置工单%s%s的挂起时间';
    $Self->{Translation}->{'Change Priority of %s%s'} = '变更工单%s%s的优先级';
    $Self->{Translation}->{'Change Responsible of %s%s'} = '变更工单%s%s的负责人';
    $Self->{Translation}->{'Manage Master/Slave status for %s%s'} = '';
    $Self->{Translation}->{'Set Master/Slave Value'} = '设置 主/从 值';
    $Self->{Translation}->{'Text will also be received by:'} = '内容也将被以下人员接收到：';

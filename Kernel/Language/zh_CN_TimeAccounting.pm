...
sub Data {
    ...
    $Self->{Translation}->{'Go to this date'} = '跳转这一天';
    ...
    $Self->{Translation}->{'End Time'} = '结束时间';
    ...
    $Self->{Translation}->{'Filter for Projects'} = '项目过滤';
    $Self->{Translation}->{'Filter for Tasks'} = '任务过滤';
    $Self->{Translation}->{'Filter for Users'} = '用户过滤';
    ...
    $Self->{Translation}->{'New User Settings'} = '新建用户设置。';
    $Self->{Translation}->{'Edit User Settings'} = '编辑用户设置';
    $Self->{Translation}->{'Comments'} = '注释';

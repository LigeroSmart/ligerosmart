...
sub Data {
    ...
    $Self->{Translation}->{'Change ITSM fields of ticket'} = 'Mudar os campos ITSM da solicitação';
    ...
    $Self->{Translation}->{'Recovery Start Time'} = 'Hora Inicial da Recuperação';
    ...
    #
    $Self->{Translation}->{'Add decision to ticket'} = 'Adicionar Decisão à Solicitação';
    $Self->{Translation}->{'Reason'} = 'Razão';
    $Self->{Translation}->{'Review Required'} = 'Revisão Requisitada';
    $Self->{Translation}->{'closed with workaround'} = 'fechada com solução de contorno';

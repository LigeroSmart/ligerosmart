...
package Kernel::System::Ticket::ArticleSearchIndex::DBMA;
...
sub ArticleSearchIndexWhereCondition {
    ...
    for my $Field (@Fields) {
        ...
        # $SQLQuery .= " MATCH ($Label.article_value) AGAINST ('$Value' IN BOOLEAN MODE)";
        $SQLQuery .= " MATCH ($Label.article_value) AGAINST ('$Value')";

        if ( $Field eq 'Fulltext' ) {
            # $SQLQuery .= " OR MATCH (st.title) AGAINST ('$Value' IN BOOLEAN MODE)";
            $SQLQuery .= " OR MATCH (st.title) AGAINST ('$Value')";
        }
    }

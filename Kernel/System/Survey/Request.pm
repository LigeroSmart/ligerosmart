...
# COMPLEMENTO - change sql to bring also valid surveys
    ...
    if($Kernel::OM->Get('Kernel::Config')->Get('Ligero::DynamicSurvey')->{'SendToValid'}){
    	push @Status, "'Valid'";
    }

	my $SurveyStatus = join ',', @Status;
...
# EO COMPLEMENTO
	...
	for my $Data (sort keys %CurrentUser){
		$Subject =~ s/<OTRS_AGENT_$Data>/$CurrentUser{$Data}/gi;
		$Body 	 =~ s/<OTRS_AGENT_$Data>/$CurrentUser{$Data}/gi;

        # filter for new rich text content
        $Body =~ s/&lt;OTRS_AGENT_$Data&gt;/$CurrentUser{$Data}/g;


	}
    $Subject =~ s/<OTRS_AGENT__.+?>/-/gi;
    $Body =~ s/<OTRS_AGENT_.+?>/-/gi;
   	#filter for new rich text content 
	$Body =~ s/&lt;OTRS_AGENT_.+?&gt;/-/gi;         

	#END COMPLEMENTO NEW TAGS 
...
sub _GetRequestRecipient {
    ...
    if ( !$ToString ) {
        ...
        my %Article = $Kernel::OM->Get('Kernel::System::Ticket::Article')->BackendForArticle( %{$Articles[0]} )->ArticleGet( %{$Articles[0]} );

...
package Kernel::System::Stats::Static::General_Agents_Stats;
...
sub Run {
    ...
    for (my $b = 0; $b < $len; $b = $b + 1)
    {
        ...
        if ( IsHashRefWithData( \%Ticket ) ) {
            ...
            if ( IsHashRefWithData( \%FirstResponse ) ) {
                ...
                $AgentData{$Ticket{Owner}}->{NumberOfEmails} += $FirstResponse{TotalEmailSendToCustomerUser};
                $AgentData{$Ticket{Owner}}->{TotalTimeFirstResponseInMin} += $FirstResponse{FirstResponseInMin} || 0;
                $AgentData{$Ticket{Owner}}->{TotalCountFirstResponseInMin}++;
            ...
            if ( IsHashRefWithData( \%TicketGetClosed ) ) {
                ...
                $AgentData{$Ticket{Owner}}->{TotalTimeSolutionInMin} += $TicketGetClosed{SolutionInMin} || 0;
                $AgentData{$Ticket{Owner}}->{TotalCountSolutionInMin}++;
    ...
    AGENT:
    for my $Agent ( sort keys %AgentData ) {        

        if ( $AgentData{$Agent}->{TotalTimeFirstResponseInMin} && $AgentData{$Agent}->{TotalCountFirstResponseInMin} ) {
            my $CalcAverageFirstResponseInMin = $AgentData{$Agent}->{TotalTimeFirstResponseInMin}/$AgentData{$Agent}->{TotalCountFirstResponseInMin};
            $AgentData{$Agent}->{AverageFirstResponseInMin} = sprintf("%d", $CalcAverageFirstResponseInMin);
        }

        if ( $AgentData{$Agent}->{TotalTimeSolutionInMin} && $AgentData{$Agent}->{TotalCountSolutionInMin} ) {
            my $CalcAverageSolutionInMin = $AgentData{$Agent}->{TotalTimeSolutionInMin}/$AgentData{$Agent}->{TotalCountSolutionInMin};
            $AgentData{$Agent}->{AverageSolutionInMin} = sprintf("%d", $CalcAverageSolutionInMin);
        }

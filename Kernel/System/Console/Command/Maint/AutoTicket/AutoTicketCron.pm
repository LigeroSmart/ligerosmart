...
sub Run {
  ...
  my $AutoTicketID = $Self->GetOption('id');

    #$TNSecuritySecs = 600000;

	# Obtain the list of all valids AutoTickets
	my %AutoTicketData;
  
  if(!$AutoTicketID){
      %AutoTicketData = $Kernel::OM->Get('Kernel::System::AutoTicket')->AutoTicketList( Valid => 1 );
  } else {
    $AutoTicketData{$AutoTicketID} = '';
  }
...
sub _scheduleIt{
    ...
    my $nonBday = $Kernel::OM->Get("Kernel::System::Time")->WorkingTime(
        StartTime => $Kernel::OM->Get("Kernel::System::Time")->TimeStamp2SystemTime(
           String => _ConvertToTimeZone(Date => "$Year-$Month-$Day $ParamObject{AutoTicket}->{Hour}:$ParamObject{AutoTicket}->{Minutes}:00", TZ => 'America/Sao_Paulo'),
            ) - 1,
    ...
    #print "nonBday ".$nonBday;

    #print "Teste ".Dumper($Kernel::OM->Get("Kernel::System::Time")->TimeStamp2SystemTime(
    ...
    #print STDOUT "\n Ticket should be create on $NDay/$NMonth/$NYear $NHour:$NMin $NWday $secsToRewind $TNSecuritySecs";

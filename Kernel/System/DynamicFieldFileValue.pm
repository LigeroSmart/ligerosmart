...
#    );  
    ...
    $Param{FormID} = $Param{FormID}||0;
    
	# a workaround for postrgreSQL
	 $Kernel::OM->Get('Kernel::System::DB')->Prepare(
        SQL   => "SELECT count(*) FROM df_attachment where article_id=\'".$Param{ObjectID}."\' AND content_id='$FieldID'",
    );  
...
#	if($Param{Plik} && $Count[0]==0){
	if($Param{Plik}){
        ...
        # check if other exists, delete if yes
		$Kernel::OM->Get('Kernel::System::DB')->Do(
			SQL => 'DELETE FROM df_attachment WHERE '
				. ' article_id = ? AND content_id=?',
			Bind => [
		...
		$Kernel::OM->Get('Kernel::System::DB')->Do(
			...
			Bind => [
				...
				\$Param{Plik}{Content}, \$FieldID, \$Param{FormID}
...
sub ValueGet {
	...
	my $TicketID = $ParamObject->GetParam( Param => 'TicketID' )||$Param{ObjectID};
	...
	my $DynamicField = $Kernel::OM->Get('Kernel::System::DB')->SelectAll(
			SQL   => "select id,filename,content,content_type,content_id from df_attachment where article_id=? and deleted=0 AND content_id=?",
			Bind => [
#				\$TicketID,\$FieldID
				\$ObjectID,\$FieldID
			],
    ...
    for my $DataRow ( @$DynamicField ) {
        #opgk KG
        if(defined $BaselinkSplit[2] && $BaselinkSplit[2] eq 'index.pl?'){
            $Action = 'AgentDFFileAttachment';
        }elsif(defined $BaselinkSplit[2] && $BaselinkSplit[2] eq 'customer.pl?'){
        ...
        my $ShowImageAsLinkOnly;
        if($Config{ShowImageAsLinkOnly} && $Config{ShowImageAsLinkOnly} eq 'yes'){
            $ShowImageAsLinkOnly=1;
        }

        my $ShowDeleteOnCustomer;
        if($Config{ShowDeleteOnCustomer} && $Config{ShowDeleteOnCustomer} eq 'yes'){
            $ShowDeleteOnCustomer=1;
        }

        if(@$DataRow[3] =~ /^image\// && !$ShowImageAsLinkOnly){
            $Content = "<div class=\"LigeroServiceCatalogContainer\"><div class=\"LigeroServiceCatalogDiv\"><img class=\"LigeroServiceCatalogImg\" src='".$BaseLink."Action=".$Action.";FieldID=$TicketID;ObjectID=@$DataRow[0]' /></div></div>";
        } else {
            $Content = @$DataRow[1];
        }

        $ValueText = "<a href='".$BaseLink."Action=".$Action.";FieldID=$TicketID;ObjectID=@$DataRow[0]'>".$Content."</a>";
        if($BaselinkSplit[2] eq 'index.pl?' || ($BaselinkSplit[2] eq 'customer.pl?' && $ShowDeleteOnCustomer)){
            $ValueText .= "<a href='".$BaseLink."Action=".
                $Action.";Subaction=Delete;BaseAction=$qs;TicketID=$TicketID;FieldID=$FieldID;ObjectID=$TicketID'> (".
                $Kernel::OM->Get('Kernel::Output::HTML::Layout')->{LanguageObject}->Translate('Delete').")</a>";
        }
        #opgk KG
		if ( !$Kernel::OM->Get('Kernel::System::DB')->GetDatabaseFunction('DirectBlob') ) {
...
sub ValueDelete {
	...
	if($DriverData->{File}->{DeleteFileDB} eq 1){
		...
		# content
			SQL => 'DELETE FROM df_attachment where article_id=? and content_id=?',
			Bind => [ \$Param{ObjectID}, \$Param{FieldID}],
		);
	}else{
		return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
			SQL => 'UPDATE df_attachment SET deleted=1 WHERE content_id=? and article_id=?',

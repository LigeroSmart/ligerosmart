...
# --
# $Id: SystemMonitoring.pm,v 1.7 2010-02-15 18:16:06 ub Exp $
# --
...
sub new {
    ...
    # get needed objects
    for (qw(ConfigObject LogObject TicketObject TimeObject)) {
        ...
        )
    {
        ...
        if ( $Self->{State} && $Self->{Host} ) {
            ...
            if ( my $TicketID = ( $Self->{TicketObject}->TicketSearch(%query) )[0] ) {
                ...
                $Param{GetParam}
                    ->{ 'X-OTRS-FollowUp-ArticleKey' . $Self->{Config}{'FreeTextState'} } = 'State';
                $Param{GetParam}
                    ->{ 'X-OTRS-FollowUp-ArticleValue' . $Self->{Config}{'FreeTextState'} }
                    = $Self->{State};
                ...
                for (qw(Host Service)) {
                    $Param{GetParam}->{ 'X-OTRS-TicketKey' . $Self->{Config}{ 'FreeText' . $_ } }
                        = $_;
                    $Param{GetParam}->{ 'X-OTRS-TicketValue' . $Self->{Config}{ 'FreeText' . $_ } }
                        = $Self->{$_};
                }

                # Set Article Free Field for State
                $Param{GetParam}->{ 'X-OTRS-ArticleKey' . $Self->{Config}{'FreeTextState'} }
                    = 'State';
                $Param{GetParam}->{ 'X-OTRS-ArticleValue' . $Self->{Config}{'FreeTextState'} }
                    = $Self->{State};
                ...
                $LogMessage = 'Mail Dropped, no matching ticket found, no open on this state '
                    . $LogMessage;
            }
        }
        else {

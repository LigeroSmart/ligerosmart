...
# --
# $Id: SystemMonitoring.pm,v 1.8 2010-02-15 19:43:54 ub Exp $
# --
        ...
        if ( $Self->{State} && $Self->{Host} ) {
            ...
            if ($TicketID) {
                ...
                for (qw(Host Service)) {
                    ...
                    $Param{GetParam}->{ 'X-OTRS-TicketValue' . $TicketFreeTextNumber }
                        = $Self->{$_};
                }
                ...
                $Param{GetParam}->{ 'X-OTRS-ArticleValue' . $ArticleFreeTextNumber }
                    = $Self->{State};

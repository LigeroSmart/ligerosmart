...
sub ValueSet {
    ...
    #    Priority => 'error',
    #    Message  => "ENTROU NO VALUE SET 1 ".Dumper($Param{DynamicFieldConfig})." SELF ".Dumper($Self->{'DynamicField_'.$Param{DynamicFieldConfig}->{Name}}),
    #);
	
    my $Success = $Kernel::OM->Get('Kernel::System::DynamicFieldFileValue')->ValueSet(
        Plik  => $Self->{'DynamicField_'.$Param{DynamicFieldConfig}->{Name}},
        ObjectID => $Param{ObjectID},
        FieldID  => $Param{DynamicFieldConfig}->{ID},
        FormID => $Self->{FormID},
    );
...
sub EditFieldValueGet {
        ...
        )
    {
		...
		if($Param{ParamObject}->GetParam( Param => $FieldName )){
            ...
            #$Kernel::OM->Get('Kernel::System::Log')->Log(
            #    Priority => 'error',
            #    Message  => "FILE CONTENT ".$FieldName,
            #);

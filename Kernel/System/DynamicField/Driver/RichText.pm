...
Kernel::System::DynamicField::Driver::RichText
...
sub EditFieldRender {
    ...
    my $isCustomerInterface = ($Kernel::OM->Get('Kernel::Output::HTML::Layout')->{Baselink} =~ m/customer\.pl/ig) ? 1 : 0;
    my $MaxLength  = $Param{MaxLength} // $Self->{MaxLength};
    my $HTMLString = "";
    if ($isCustomerInterface) {
      $HTMLString = "<style> .cke_editor_$FieldName { margin-left: 135px !important; } </style>"
    }
    $HTMLString .= <<"EOF";
    
<textarea class="$FieldClass RichText" id="$FieldName" name="$FieldName" title="$FieldLabelEscaped" rows="15" cols="70" data-maxlength="$MaxLength">$ValueEscaped</textarea>
EOF

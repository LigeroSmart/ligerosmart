...
sub ValueLookupArray {
    ...
    for my $Key ( @Keys ){
        ...
        my $SQL = 'SELECT '
            . $Param{DynamicFieldConfig}->{Config}->{DatabaseFieldTableView}
            . ' FROM '
            . $Param{DynamicFieldConfig}->{Config}->{DatabaseTable}
            . $QueryCondition;
...
sub EditFieldRender {
    ...
    <div class="InputField_Container W95pc">
        <input id="$AutoCompleteFieldName" type="text" style="margin-bottom:2px;" />
END

    $HTMLString .= <<"END"; 
        <div class="Clear"></div>
        <div class="RemoteDBListContainer_$FieldName">
            <table id="table_$FieldName" class="Hidden Small DataTable RemoteDBList_$FieldName">
                <thead>
END

#    $HTMLString .= <<"END";                 
#END
    my @HaderTableArrayRef = [];
    if ( defined $Param{DynamicFieldConfig}->{Config}->{ShowKeyInTitle} ) {
        if ( ref $Param{DynamicFieldConfig}->{Config}->{ShowKeyInTitle} eq 'ARRAY' ) {
            @HaderTableArrayRef = $Param{DynamicFieldConfig}->{Config}->{DatabaseFieldSearch};
        }
        else {
            @HaderTableArrayRef = split( ',', $Param{DynamicFieldConfig}->{Config}->{DatabaseFieldTableView} )
        }
    }

    for my $Key ( @HaderTableArrayRef ) {
        $HTMLString .= <<"END";
                    <th class="$Key">$Key</th>
END
    }
...
END
    ...
    for my $Key ( @{ $SelectedValuesArrayRef } ) {
        next if (!$Key);
        $ValueCounter++;
        my $row = $Self->ValueLookupArray(
            %Param,
            Key => $Key,
        );
        $HTMLString .= <<"END";
                    <tr id="row_$ValueFieldName$ValueCounter">
END
...
END
        ...
        $HTMLString .= <<"END";
                        <td class="Delete">
                            <a href="#" class="RemoteDBTableDelete" data-counter=$ValueCounter aria-label="Click to delete this row." tabindex="0" >
...
END
        $Param{LayoutObject}->AddJSOnDocumentComplete( Code => <<"END");
            ...
            // var deleteLink = document.querySelector('a.RemoteDBTableDelete');
            // Adiciona o manipulador de eventos ao elemento
            \$('a.RemoteDBTableDelete').bind('click', function() {
                var stringID = '#' + '$ValueFieldName' + \$(this).data("counter");
                \$(stringID).parent().remove();
                ...
                linha.remove(); // Remove a linha do DOM
                if (\$('.InputField_Selection > input[name=$FieldName]').length == 0) {
                    \$('#$ContainerFieldName').hide().append(
                        '<input class="InputField_Dummy" type="hidden" name="$FieldName" value="" />'
                    );
                }                
                if (\$('#table_$FieldName').find('tbody tr').length === 0) {
                    \$('#table_$FieldName').hide();
                }

                return false;                
            }); 
            \$('#table_$FieldName').each(function() {
                if (\$(this).find('tbody tr').length) {
                    \$('#table_$FieldName').show();                  
                }
            });                                   
...
END
    ...
    \$('#$AutoCompleteFieldName').autocomplete({
        ...
        select: function (Event, UI) {
            ...
            var table = document.getElementsByClassName('RemoteDBList_$FieldName')[0];
            var tbody = table.getElementsByTagName('tbody')[0];
            var data = UI.item.table;
            // Limpar o conteúdo da tabela, se necessário
            // tbody.innerHTML = '';
            // Iterar pelo array
            // Criar uma nova linha <tr>
            var row = document.createElement('tr');

            // Iterar pelos valores da linha
            data.forEach(function(value) {
                // Criar um novo elemento <td>
                var cell = document.createElement('td');

                // Definir o conteúdo da célula
                cell.textContent = value;

                // Adicionar a célula como filho da linha
                row.append(cell);
            });
            var celldel = document.createElement('td');
            celldel.innerHTML = '<td class="Delete">'
                +'<a href="#" class="RemoteDBTableDelete" data-counter='
                + countID
                + ' aria-label="Click to delete this row." tabindex="0" >'
                +'<i class="fa fa-trash"></i>'
                +'</a>'
                +'</td>';
            row.append(celldel);
            ...
            tbody.append(row);
            \$('.RemoteDBList_$FieldName').each(function() {
                if (\$(this).find('tbody tr').length) {
                    \$(this).show();
                }
            });

...
# Copyright (C) 2019 Complemento, http://complemento.net.br
# --
...
package Kernel::GenericInterface::Transport::LigeroEasyConnector::REST;
...
sub RequesterPerformRequest {
    ...
    # COMPLEMENTO: Possibility to interpolate special information from your Company
    # Declare your special variable at Config.pm or at a ZZZZSomething.pm file at
    # /opt/otrs/Kernel/Config/Files directory such as:
    # $Self->{CompanyIntegrations}->{MyVar} = 'some value';
    # The interpolation occurs on Username or Password (token generation)
    my $CompanyIntegrations = $Kernel::OM->Get('Kernel::Config')->Get('CompanyIntegrations');
    # Replace config options.
    my $Tag = '<CompanyIntegrations_';
    ...
    if ( IsHashRefWithData( $Self->{TransportConfig}->{Config}->{AdditionalHeaders} ) ) {
        ...
        for my $AdditionalHeader ( sort keys %AdditionalHeaders ) {
            $AdditionalHeaders{$AdditionalHeader} =~ s{$Tag(.+?)>}{
                my $Key   = $1;
                my $Value = $CompanyIntegrations->{$1} // '';
                $Value;
            }egx;

...
Core.Agent.Admin.CustomerContract = (function (TargetNS) {
    ...
    TargetNS.Init = function() {
        ...
        $('.DataTable').each(function() {
            if ($(this).find('a.Edit').length) {
                $('.Edit').bind('click', function() {
                    //alert($(this).closest('tr').prop('id')); 
                    alert(Core.Config.Get('AdminCustomerContract'));
                    /**var TemplateHTML = Core.Template.Render('Agent/AdminCustomerContract/AdminCustomerContract', {
                        //'TicketAppointments': Core.Config.Get('AdminCustomerContract')
                        'TicketAppointments': Core.Config.Get('AdminCustomerContract')
                    }),                              
                    $RuleObj = $(TemplateHTML);                 
            
                    Core.UI.Dialog.ShowContentDialog($RuleObj, Core.Language.Translate("Edit franchise rule"), '20%', 'Center', true);*/
